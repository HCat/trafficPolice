//
//  UICollectionView+Lr_Placeholder.m
//  trafficPolice
//
...
@implementation UICollectionView(Lr_Placeholder)
...
- (void)checkEmpty{
    ...
    if ([dataSource respondsToSelector:@selector(numberOfSectionsInCollectionView:)]) {
        sections = [dataSource numberOfSectionsInCollectionView:self] - 1;//获取当前CollectionView组数
    }
    
    for (NSInteger i = 0; i <= sections; i++) {
        NSInteger rows = [dataSource collectionView:self numberOfItemsInSection:i];//获取当前CollectionView各组行数
        if (rows) {
